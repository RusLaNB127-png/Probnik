...
// Мини-кольцо прогресса сотрудника
...
function renderChecklist(){
  ...
  if(rop){
    ...
    document.getElementById('clSub').textContent =
      `Вы вошли как ${u.name} · ${u.role}. Выберите сотрудника слева, чтобы открыть его чек-лист. Общий трекер — в Дашборде РОП.`;
    actions.innerHTML = `
    ...
    renderRopChecklist(content);
    document.getElementById('clAddTask').onclick = ()=>openTaskModal(null, state.checklistMgr || null);
...
// РОП: список сотрудников слева + чек-лист выбранного справа
function renderRopChecklist(content){
  if(!MANAGERS.find(m=>m.id===state.checklistMgr)) state.checklistMgr = MANAGERS.length ? MANAGERS[0].id : '';
  content.innerHTML = `
    <div class="cl-grid">
      <aside class="cl-team-card">
        <h3 class="cl-team-title">Сотрудники ОП</h3>
        <div class="cl-team" id="clEmpList"></div>
      </aside>
      <section class="cl-main-card" id="clRopBody"></section>
    </div>`;

  const sel = state.checklistMgr;
  document.getElementById('clEmpList').innerHTML = MANAGERS.map(m=>{
    const st = taskStats(m.id);
    return `<button class="cl-emp ${m.id===sel?'active':''}" data-mgr="${m.id}">
      <div class="avatar">${m.short||mkShort(m.name)}</div>
      <div class="cl-emp-meta"><b>${escapeHtml(m.name)}</b>
        <span>${st.done}/${st.total}${st.overdue?` · ${st.overdue} просроч.`:''}</span></div>
      <div class="cl-emp-ring">${clRing(st.pct)}</div>
    </button>`;
  }).join('');
  document.querySelectorAll('#clEmpList [data-mgr]').forEach(b=>b.onclick = ()=>{
    state.checklistMgr = b.dataset.mgr;
    renderRopChecklist(content);
  });

  const body = document.getElementById('clRopBody');
  const mgr = MANAGERS.find(m=>m.id===sel);
  if(!mgr){ body.innerHTML = `<div class="dash-empty">Нет сотрудников</div>`; return; }
  const st = taskStats(sel);
  body.innerHTML = `
    <div class="cl-body-head">
      <div><h3>${escapeHtml(mgr.name)}</h3>
        <div class="cl-body-sub">${st.done} из ${st.total} выполнено${st.overdue?` · ${st.overdue} просрочено`:''}</div></div>
      <button class="btn btn-sm ic-inline" id="clAddForSel">${icon('plus',14)} Задача</button>
    </div>
    ${taskStatsBar(sel)}
    ${taskTableHTML(tasksFor(sel), { showOwner:false, canEdit:true })}`;
  document.getElementById('clAddForSel').onclick = ()=>openTaskModal(null, sel);
  wireTaskTable(body);
}
